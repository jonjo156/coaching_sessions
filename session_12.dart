...
class HomePage extends StatelessWidget {
  ...
  Widget build(BuildContext){
    var oneUsdValue = 1 / double.parse(crypto.price);
    return Scaffold(
      body: Column(
    ...
    child: Padding(
      ...
      child: Column(
        ...
        child: Column(children: [
          CryptoRow(leftText: '1', rightText: 'USD', assetLogo: Image.asset('assets/images/bitcoin.png', height: 20,)),
                  Divider(color: Colors.black,),
        CryptoRow(leftText: oneUsdValue.toStringAsFixed(6), rightText: 'BTC', assetLogo: Image.asset('assets/images/bitcoin.png', height: 20,))
        ],),
        ),
        TextField()
        ],
...
class CryptoRow extends StatelessWidget {
  ...
  final Widget assetLogo;
  const CryptoRow({super.key, required this.leftText, required this.rightText, required this.assetLogo});
  ...
  Widget build(BuildContext context) {
    return  Padding(
      ...
      child: Row(
        ...
        children: [Text(leftText, style: GoogleFonts.openSans(fontSize: 24, color: Color(0xff1E2329), fontWeight: FontWeight.w600),),
      Container(decoration: BoxDecoration(borderRadius: BorderRadius.circular(40), color: Colors.grey[200]), child: Padding(padding: EdgeInsets.all(8), child: Row(children: [
      assetLogo,
        const SizedBox(width: 4,), Text(rightText, ), const SizedBox(width: 8,), Icon(Icons.arrow_drop_down)]),))
      ]),
    );
  }
}
